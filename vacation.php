...
$RegistroDesde= $_GET["registroDesde"];	
?>
...
if (empty($city)) {	
    $queryDetail = ("SELECT villa.VillaId, villa.Name, villa.pricefrom, villa.priceto, villadetail.VillaId, villadetail.Country, villadetail.quantityroom, villadetail.quantitybath, villadetail.PathVilla FROM villa, villadetail WHERE villadetail.VillaId=villa.VillaId AND villadetail.Country='$country' AND villadetail.status=0");		
	
	if ( (!empty($bedrooms)) ) {	
		$queryDetail = ("SELECT villa.VillaId, villa.Name, villa.pricefrom, villa.priceto, villadetail.VillaId, villadetail.Country, 
villadetail.quantityroom, villadetail.quantitybath, villadetail.PathVilla FROM villa, villadetail WHERE quantityroom>= '$bedrooms' AND villadetail.status=0");
		if ($bedrooms == 4 ) {
			$queryDetail = ("SELECT villa.VillaId, villa.Name, villa.pricefrom, villa.priceto, villadetail.VillaId, villadetail.Country,  villadetail.quantityroom, villadetail.quantitybath, villadetail.PathVilla FROM villa, villadetail WHERE quantityroom>= '$bedrooms' AND villadetail.status=0");	
			}
	}	
	if ( (!empty($bathrooms)) ) {								
		$queryDetail = ("SELECT villa.VillaId, villa.Name, villa.pricefrom, villa.priceto, villadetail.VillaId, villadetail.Country, 
villadetail.quantityroom, villadetail.quantitybath, villadetail.PathVilla FROM villa, villadetail WHERE quantitybath>='$bathrooms' AND villadetail.status=0");	
		if ($bathrooms == 4 ) {
			$queryDetail = ("SELECT villa.VillaId, villa.Name, villa.pricefrom, villa.priceto, villadetail.VillaId, villadetail.Country, villadetail.quantityroom, villadetail.quantitybath, villadetail.PathVilla FROM villa, villadetail WHERE quantitybath>='$bathrooms' AND villadetail.status=0");	
			}							
	}			
	if (  (!empty($country)) and (!empty($bedrooms))  ) {	
		$queryDetail = ("SELECT villa.VillaId, villa.Name, villa.pricefrom, villa.priceto, villadetail.VillaId, villadetail.Country, 
villadetail.quantityroom, villadetail.quantitybath, villadetail.PathVilla FROM villa, villadetail WHERE villadetail.VillaId=villa.VillaId AND villadetail.Country='$country' AND villadetail.status=0 AND quantityroom>= '$bedrooms'");	
		if ($bedrooms == 4 ) {
			$queryDetail = ("SELECT villa.VillaId, villa.Name, villa.pricefrom, villa.priceto, villadetail.VillaId, villadetail.Country, villadetail.quantityroom, villadetail.quantitybath, villadetail.PathVilla FROM villa, villadetail WHERE villadetail.VillaId=villa.VillaId AND villadetail.Country='$country' AND villadetail.status=0 AND quantityroom>= '$bedrooms'");	
			}
	}	
	if (  (!empty($country)) and (!empty($bathrooms)) ) {							
		$queryDetail = ("SELECT villa.VillaId, villa.Name, villa.pricefrom, villa.priceto, villadetail.VillaId, villadetail.Country, 
villadetail.quantityroom, villadetail.quantitybath, villadetail.PathVilla FROM villa, villadetail WHERE villadetail.VillaId=villa.VillaId AND villadetail.Country='$country' AND villadetail.status=0 AND quantitybath>='$bathrooms'");							
		if ($bathrooms == 4 ) {
		$queryDetail = ("SELECT villa.VillaId, villa.Name, villa.pricefrom, villa.priceto, villadetail.VillaId, villadetail.Country, 
villadetail.quantityroom, villadetail.quantitybath, villadetail.PathVilla FROM villa, villadetail WHERE villadetail.VillaId=villa.VillaId AND villadetail.Country='$country' AND villadetail.status=0 AND quantitybath>='$bathrooms'");
			}							
	} 
	if (  (!empty($country)) and (!empty($bedrooms)) and (!empty($bathrooms)) ) {	
		$queryDetail = ("SELECT villa.VillaId, villa.Name, villa.pricefrom, villa.priceto, villadetail.VillaId, villadetail.Country, 
villadetail.quantityroom, villadetail.quantitybath, villadetail.PathVilla FROM villa, villadetail WHERE villadetail.VillaId=villa.VillaId AND villadetail.Country='$country' AND villadetail.status=0 AND quantityroom>= '$bedrooms' AND quantitybath>='$bathrooms'");
		if ($bedrooms == 4 ) {
		$queryDetail = ("SELECT villa.VillaId, villa.Name, villa.pricefrom, villa.priceto, villadetail.VillaId, villadetail.Country, 
villadetail.quantityroom, villadetail.quantitybath, villadetail.PathVilla FROM villa, villadetail WHERE villadetail.VillaId=villa.VillaId AND villadetail.Country='$country' AND villadetail.status=0 AND quantityroom>= '$bedrooms' AND quantitybath>='$bathrooms'");
			}
...
villadetail.quantityroom, villadetail.quantitybath, villadetail.PathVilla FROM villa, villadetail WHERE villadetail.VillaId=villa.VillaId AND villadetail.Country='$country' AND villadetail.status=0 AND quantityroom>= '$bedrooms' AND quantitybath>='$bathrooms'");
	...
	if (!empty($villaName)) {
    	$queryDetail = ("SELECT villa.VillaId, villa.Name, villa.pricefrom, villa.priceto, villadetail.VillaId, villadetail.Country, 
villadetail.quantityroom, villadetail.quantitybath, villadetail.PathVilla FROM villa, villadetail, imagegallery WHERE villadetail.VillaId=villa.VillaId AND villa.Name like % '$country' % AND villadetail.status=0");
    }
}
...
?>
  ...
  <?php
		...
		if (!empty($RegistroDesde)) {
			$buscarSiguiente = " ORDER BY villa.VillaId ASC LIMIT 10 OFFSET " . $RegistroDesde;
			$queryDetail = $queryDetail . $buscarSiguiente;
			//echo $queryDetail;
			$resultadoDetail=mysql_query($queryDetail,$dbConn);		
		}  		
	?>
...
<div class="container-fluid" id="tourpackages-carousel">   
    ...
    ?>   
    <!--Paginacion, se debe calcular el numero de paginas a mostrar de 20 thumbnail c/u-->  
    <?php
    $paginas = ceil($numero_total_filas / 10);
	?>
      
<nav aria-label="Page navigation" style="text-align:center;">
  <ul class="pagination">
    <li>
      <a href="vacation.php?registroDesde=0" aria-label="Previous">
        <span aria-hidden="true">&laquo;</span>
      </a>
    </li>
    <li><a href="vacation.php?registroDesde=0">1</a></li>
    <?php
	for ($x = 2; $x <= $paginas; $x++) {
	?>
    <?php
...
?> 
		<li><a href="vacation.php?registroDesde=<?php echo $aPartirDe?>"><?php echo $x ?></a></li>
    <?php
	}
	?>     
    <li>
      <a href="vacation.php?registroDesde=<?php echo $aPartirDe?>" aria-label="Next">
