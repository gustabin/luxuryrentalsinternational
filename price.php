...
<script Language="JavaScript">
    ...
    function Modificar(id) {		
		window.location.href='editPrice.php?id=' + id;		
    }
	
	//FUNCIÓN PARA ELIMINAR
    function Eliminar(id) {
		window.location.href='confirmarEliminarPrice.php?id=' + id;
    }
...
<script type="text/javascript">
    ...
    $("body").on('submit', '#formBuscar', function(event) {
		...
		if ($('#formBuscar').valid()) {
			$.ajax({
				type: "POST",
				url: "formBuscarProcesarPrice.php",
				dataType: "json",
				data: $(this).serialize(),
				success: function(respuesta) {
					...
					if (respuesta.exito == 1) {
						...
						setTimeout(function() {
						$('#exito1').hide();
						window.location.href='price.php?id=<?php echo $villaId;?>'; 
					  }, 1000);
					}
 ...
 <!-- .................................... $Titulo .................................... -->
      <h2 class="section-title">
        Price
        <small>by season to <?php echo $_SESSION['villaName']; ?></small>
      </h2>
      ...
      <ul class="breadcrumb">      
        <li><span id="titulo"></span></li>
        	<form  method="post" name="formBuscar" id="formBuscar">
        		<div class="controls">
          			<input type="text" id="nombre" name="nombre" style="width: 20%;"  placeholder="Season" />	  
        		
           			<button id="search-btn" type="submit" name="submit" class="btn-main"><i class="fa fa-search" aria-hidden="true"></i>
 Search </button>
      		</form>

                    <a href="priceAdd.php?id=<?php echo $villaId;?>">
    ...
    <div class="container">
    	    <div id="sresults" class="col-md-12">
   			<table id="sTable" class="tablesorter table table-bordered table-hover" style="border:1px solid #ECF0F1">
        		...
        		<tr style="background:#f5f5f5">
                    <td colspan="12" style="text-align: right">
                        <span class="span_required"id="ErrorBoton" style="display: none; font-size: 15px; float: left">
                            Choose the season for edit or delete
                        </span>
                ...
                <tr>                    
                    <th width="25%" class="header" style="text-align: center">Season</th>
					<th width="20%" class="header" style="text-align: center">Date From</th>
                    <th width="20%" class="header" style="text-align: center">Date To</th>
                    <th width="5%" class="header" style="text-align: center">Quantity Room</th>
                    <th width="5%" class="header" style="text-align: center">Type</th>
                    <th width="10%" class="header" style="text-align: center">Price</th>
                    <th width="5%" class="header" style="text-align: center">Min Nights</th>
...
<?php
		...
		if (!empty($_SESSION['nombreSeason'])) 
		{	
			$name = $_SESSION['nombreSeason'];
			$query = ("SELECT * FROM settingprice WHERE season LIKE '%$name%' AND villaid='$villaId'");
		}
		else
		{
			$query = ("SELECT * FROM settingprice WHERE villaid='$villaId'");
		}
//var_dump($query);
      ...
      {
		$seasonName = $data_his['season'];
		$datefrom = $data_his['seasondatefrom'];	
		$dateto = $data_his['seasondateto'];
		$quantityroom = $data_his['definedroom'];
		$type = $data_his['pricetype'];
		$price = $data_his['price'];
		$minnights = $data_his['minimument'];
		$Id = $data_his['settingpriceid'];	
	    ?>
        		<tr>
            		<td><?php echo $seasonName;?></td>
                    <td><?php echo $datefrom;?></td> 
                    <td><?php echo $dateto;?></td>
                    <td><?php echo $quantityroom;?></td>
                    <td><?php echo $type;?></td>
                    <td><?php echo $price;?></td>
                    <td><?php echo $minnights;?></td>
